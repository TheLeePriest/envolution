...
/**
 * AWS-specific environment schema
 */
export const AWSEnvironmentSchema = z.object({
  ...
  MAX_TOKENS: z.string().transform(Number).pipe(z.number().positive()).optional(),
  REFILL_RATE_PER_MS: z.string().transform(Number).pipe(z.number().positive()).optional(),
  ...
  LAMBDA_TIMEOUT: z.string().transform(Number).pipe(z.number().positive()).default('30'),
  LAMBDA_MEMORY_SIZE: z.string().transform(Number).pipe(z.number().positive()).default('128'),
});
