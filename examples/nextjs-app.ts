...
const env = EnvironmentConfig.getInstance(
	AppSchema as z.ZodType<Record<string, unknown>>,
);
...
export async function GET() {
	try {
		const apiUrl = env.getRequired("NEXT_PUBLIC_API_URL", "API URL");
		const jwtSecret = env.getRequired("JWT_SECRET", "JWT authentication");
...
export async function getServerSideProps() {
	const databaseUrl = env.getRequired("DATABASE_URL", "Database connection");
