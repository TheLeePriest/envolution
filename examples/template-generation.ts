...
const CustomAppSchema = z.object({
	...
	REDIS_URL: z
		...
		.url()
		.optional()
		.describe("Redis connection URL (optional)"),
	SENDGRID_API_KEY: z
		.string()
		.optional()
		.describe("SendGrid API key for email (optional)"),
});
...
console.log(
	`📁 TypeScript types: ${allTemplates.types.outputFile || "not saved"}`,
);
console.log(
	`📁 JSON schema: ${allTemplates.jsonSchema.outputFile || "not saved"}`,
);
