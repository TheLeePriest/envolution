...
export class EnvironmentValidationError extends Error {
    constructor(message, missingVars = [], invalidVars = [], context = undefined) {
        super(message);
...
export class EnvironmentConfig extends EventEmitter {
    ...
    getRequired(key, context) {
        ...
        if (value === undefined || value === null) {
            ...
            throw new EnvironmentValidationError(`${String(key)} environment variable is required${contextMsg}`, [String(key)], [], context);
        }
        return value;
    }
    getAll() {
    ...
    validateStartup() {
        ...
        try {
            ...
            if (error instanceof z.ZodError) {
                ...
                const invalidMsg = invalidVars.length > 0
                    ? `\nInvalid variables: ${invalidVars
                        .map((v) => `${v.name} (expected ${v.expected}, got '${v.value}')`)
                        .join(', ')}`
                    : '';
                throw new EnvironmentValidationError(`Environment validation failed at startup.${missingMsg}${invalidMsg}`, missingVars, invalidVars);
            }
            throw error;
        }
    }
            ...
            if (error instanceof z.ZodError) {
                ...
                const invalidMsg = invalidVars.length > 0
                    ? `\nInvalid variables: ${invalidVars
                        ...
                        .join(', ')}`
                    : '';
                throw new EnvironmentValidationError(`Environment validation failed.${missingMsg}${invalidMsg}`, missingVars, invalidVars);
            }
            throw error;
        }
    }
